//Chat application between TCP/IP client and server 
#undef UNICODE
...
int __cdecl mainServer(void)
{
	...
	do {
		...
		if (iResult > 0) {
			...
			char message[256];
			memcpy(message, recvbuf, iResult);
			message[iResult] = '\0';
			cout << endl << "(Client):" << message << endl;

			// Echo the buffer back to the sender
			char sendMessage[256];
			cout << "(Server):"; cin >> sendMessage;
			iSendResult = send(ClientSocket, sendMessage, (int)strlen(sendMessage), 0);
...
int __cdecl mainClient()
{
	...
	do {
		...
		char sendMessage[256];
		cout << "(Client[end-exit]):"; cin >> sendMessage;
